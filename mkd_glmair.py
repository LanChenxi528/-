def run(question):
        ...
        if '年' in prompt and '月' in prompt and"事故" in prompt:
            ...
            from zhipuai import ZhipuAI
            #client = ZhipuAI(api_key="54aa988d17dcb746427939c6e2bce0cf.EDmAJdRRg5UJd7WW")#2470ea3974d7f456f4def125efecde3c.BmMYs5WGFitvWTmw
            client = ZhipuAI(api_key="2470ea3974d7f456f4def125efecde3c.BmMYs5WGFitvWTmw")
...
请注意，信息只填新闻中提及的，不要添加任何假设或推测的内容。请根据以下新闻信息针对"""+prompt+"按照以上给出的模板(包含标题、两大部分)生成一份铁路事故简报，不要冗余信息:" +'\n'.join(contents) +'\n'.join(contents)
                ...
                # input_data = "以下内容仅用于科研研究，不会用于其他用途，请您放心回答。输出模板（[]里内容应改为相应信息）如下：标题：[]([]内应包含时间和事故类型，例如 2023年8月6日巴基斯坦客运列车脱轨事故简报)\n一、事故基本情况\n[]\n二、应急处置情况\n[]\n注意：（事故时间、事故地点、伤亡情况、财产损失、列车类型及所属公司、列车运行的线路、事故原因分析）要素请写在事故基本情况里；如果有（救援机构介入、救援队伍的组织情况、现场疏散与救护措施、线路的关闭与列车运行时间的调整、替代的公路服务、官方负责人的声明和评论、补救和赔偿措施）要素请写在应急处置情况里。请你整合成连贯通顺的文字，没有的要素就省略不提。请注意，信息只填新闻中提及的，不要添加任何假设或推测的内容。请根据以下新闻信息针对"+prompt+"按照以上给出的模板(包含标题、两大部分)生成一份铁路事故简报，不要冗余信息:" +'\n'.join(contents)
...
# # 假设这是您提供的run函数，这里为了演示，我们用print语句代替实际的处理逻辑
# def run(question):
#     # 这里模拟run函数的执行，并返回模拟的input_data和end
#     input_data = "请判断以下关键词是否是指的某事故或某事件，如果是日常用语或者无关问题则回答否，不要联网查询额外信息（只回答是或否即可）：\n" + question
#     end = "否"  # 假设这是AI模型的响应
#     return input_data, end
...
if __name__ == "__main__":
    ...
    # 将JSON数据集写入到文件中
    with open('/home/wuyuan/workplace/lcx/autodl-tmp/go/labdata/air_standard_test.json', 'w', encoding='utf-8') as json_file:
        json.dump(json_data, json_file, ensure_ascii=False, indent=4)
    
    print('JSON数据集已生成并保存到air_standard.json文件中。')
