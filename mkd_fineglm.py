...
def run(question):
        ...
        from model_loader import get_loaded_model,get_loaded_model2
        import os
...
文段润色后应该更加易读且专业，确保表述流畅、逻辑清晰。请注意，信息只填新闻中提及的，不要添加任何假设或推测的内容。请根据以下新闻信息针对"""+prompt+"按照以上给出的模板(包含标题、两大部分)生成一份铁路事故简报，不要冗余信息:" +'\n'.join(contents) +'\n'.join(contents)
                ...
                # current_length = 0
                # history=[]
                llm2=get_loaded_model2()
                response=llm2.generate_resp(input_data)
                end=response
                # model2,tokenizer2=get_loaded_model2()
                # tokenizer2.padding_side = "left"
                # input_data=input_data[0:8000]
                # response = model2.chat(tokenizer2, input_data, history=[])
                # end=response
                print(end)
...
if __name__ == "__main__":
    ...
    # 将JSON数据集写入到文件中
    with open('/home/wuyuan/workplace/lcx/autodl-tmp/go/labdata/fineglm_new.json', 'w', encoding='utf-8') as json_file:
        json.dump(json_data, json_file, ensure_ascii=False, indent=4)
    
    print('JSON数据集已生成并保存到fineglm_new.json文件中。')
