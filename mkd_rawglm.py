...
# tokenizer_path2="/home/wuyuan/workplace/lcx/autodl-tmp/LLaMA-Factory/saves/ChatGLM3-6B-Chat/lora/train_2025-02-12-10-20-58"
# # 提供已加载的模型
# def get_loaded_model2():
#     return load_model2(model_path2,tokenizer_path2)
...
文段润色后应该更加易读且专业，确保表述流畅、逻辑清晰。请注意，信息只填新闻中提及的，不要添加任何假设或推测的内容。请根据以下新闻信息针对"""+prompt+"按照以上给出的模板(包含标题、两大部分)生成一份铁路事故简报，不要冗余信息:" +'\n'.join(contents) +'\n'.join(contents)
                ...
                # current_length = 0
                # history=[]
                response=llm.generate_resp(input_data)
                # model2=get_loaded_model()
                # response=model2.generate_resp(input_data)
                end=response
                print(end)
...
if __name__ == "__main__":
    ...
    # 将JSON数据集写入到文件中
    with open('/home/wuyuan/workplace/lcx/autodl-tmp/go/labdata/rawglm_new.json', 'w', encoding='utf-8') as json_file:
        json.dump(json_data, json_file, ensure_ascii=False, indent=4)
    
    print('JSON数据集已生成并保存到rawglm_new.json文件中。')
